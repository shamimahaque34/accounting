                    ...
                    <div class="col-md-8 mx-auto">
                        <div class="card mt-3">
                            <div class="card-header">
                                <h4 class="float-start">Update User</h4>
                                ...
                                <div>
                                    <form action="{{ route('users.update',$user->id) }}" method="POST">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="text" class="form-control" name="name" required value="{{$user->name}}" data-provide="typeahead" id="the-basics" placeholder="User Name">
                                            <span class="text-danger">{{ $errors->has('name') ? $errors->first('name') : '' }}</span>
                                        </div>
                                        <div class="mb-3">
                                            <label class="form-label">Email <span class="text-danger">*</span></label>
                                            <input type="email" class="form-control" required name="email" value="{{$user->email}}" data-provide="typeahead" id="" placeholder="User Email">
                                            <span class="text-danger">{{ $errors->has('email') ? $errors->first('email') : '' }}</span>
                                        </div>
                                        <div class="mb-3">
                                            <label class="form-label"> Password <span class="text-danger">*</span></label>
                                            <input type="text" class="form-control" name="password" value="{{$user->password}}"data-provide="typeahead" id="" placeholder="{{ isset($user) ? 'Update' : 'User' }} Password {{ isset($user) ? 'if required' : '' }}">
                                        </div>
                                        <div class="mb-3">
